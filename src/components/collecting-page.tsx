...
export function CollectingPage({ activityId }: { activityId: string }) {
  ...
  return (
    <main className="flex min-h-dvh justify-center bg-[#090a0c] text-[#f8f4ed]">
      ...
      >
        {!participant && (
          <div className="fixed inset-y-0 left-1/2 z-[120] grid w-full max-w-[393px] -translate-x-1/2 place-items-center bg-black/68 px-6 backdrop-blur-[4px]">
            <section
              role="dialog"
              aria-modal="true"
              aria-label="怎么称呼你？"
        ...
        {isMemoryOpen && (
          <div className="fixed inset-y-0 left-1/2 z-[90] w-full max-w-[393px] -translate-x-1/2 overflow-hidden">
            ...
            <section
              ...
              className={`absolute inset-x-0 bottom-0 flex min-h-dvh flex-col rounded-t-[24px] bg-[#23262d] px-5 pb-[max(28px,env(safe-area-inset-bottom))] pt-[190px] shadow-[0_-24px_60px_rgba(0,0,0,0.45)] ${
